...
const Double_t eneBeam = 10.04; // MeV; nominal


const Int_t n_ch_s3 = 24; // number of rings
Double_t mu_s3[n_ch_s3]; // mean strip position
Double_t mu_s3f[n_ch_s3], mu_low_s3f[n_ch_s3], mu_high_s3f[n_ch_s3]; // limits of strip means
Double_t bin_s3f[2*n_ch_s3]; // rad
const Double_t ene_low = 0.; // qdc chn
const Double_t ene_high = 15.;
const Int_t ene_bin = (ene_high - ene_low)*50;
const Int_t chn_s3f[25] = { 1,  2,  3,  4,  5,  6,  7,  8, 10,  9, // converts plugs (1..25) into channels (1..25) (before January 2017) Jean Nippert
			   12, 11, 24, 23, 22, 21, 20, 19, 18, 17,
			   15, 16, 13, 14, 25};
...
const Double_t thetaMin_s3f = TMath::ATan (r_i_s3/z_s3f)*180./TMath::Pi (); // rad; range of kinematics lines; could also calc from bin_s3f[2*n_ch_s3] ..
const Double_t thetaMax_s3f = TMath::ATan (r_a_s3/z_s3f)*180./TMath::Pi ();
...
const Double_t thick_fwd = 10 + 0.1; // µm   CHANGE THICKNESS OF FOIL HERE
const Double_t alu_fwd = thick_fwd*TMath::Power (10, -4)*alu_density; // surface density; g/cm2
...
Int_t mask_a[25][8], mask_p[25][11];
Double_t levX_a[25][8], levY_a[25][8], levX_p[25][11], levY_p[25][11], dLevX_a[25][8], dLevX_p[25][11]; // MeV; lines gating on peak regions
Int_t maxIn_a[25][8], minIn_a[25][8], maxIn_p[25][11], minIn_p[25][11];
std::vector <Double_t> eneLow[25], eneHigh[25]; // peak region boundaries
std::vector <Int_t> idPart[25]; // level/particle identifier

TF1 *f_fitSingle;
Double_t ampl_a[25][8], mean_a[25][8], sigm_a[25][8], ampl_p[25][11], mean_p[25][11], sigm_p[25][11];
